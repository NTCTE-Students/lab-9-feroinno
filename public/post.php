...
// Проверка + Функционал Лайка Поста
if ($auth -> check() && isset($_GET['where']) && $_GET['where'] === 'like') {
    $db = new \App\Config\Database();

    $existing = $db -> read('likes', where: [
    ...
    if ($existing) {
        ...
        (new \App\Models\Like()) -> createAndSet([
            'user_id' => $auth -> user() -> getAttribute('id'),
            'likeable_type' => 'posts',
            'likeable_id' => $post -> getAttribute('id')
        ]);
...
// Конец Проверка + Функционал Лайка Поста
...
// Проверка + Функционал Лайка
if ($auth -> check() && isset($_GET['where']) && $_GET['where'] === 'like_comment' && isset($_GET['comment_id'])) {
...
<body class="max-w-6xl xl:mx-auto md:mx-10">
    ...
    <main class="mt-10 my-8 md:m-0 mx-8">
        ...
        <a class="inline-flex items-center mb-16 px-4 py-2 bg-cyan-600 text-cyan-50 hover:bg-cyan-900 md:rounded-xl" href="/post.php?id=<?php print($post -> getAttribute('id')); ?>&where=like">
            ...
            <!-- Лайк! -->
            Лайк! (<?php
                $dbase = new \App\Config\Database();
                $likes = $dbase -> read('likes', where: [
                    ['likeable_type', '=', 'posts'],
                    ['likeable_id', '=', $post -> getAttribute('id')],
                ]);
                echo count($likes);
                                ...
                                <?php if ($auth -> check()) { ?>
                                    <div>
                                        <div class="peer flex justify-between mt-4 space-x-4">
                                            ...
                                            <a href="/post.php?id=<?php print($post -> getAttribute('id')) ?>&where=like_comment&comment_id=<?php print($comment['id']); ?>" class="cursor-pointer flex items-center text-sm text-gray-500 hover:underline font-medium">
                                                <!-- Лайк -->
                                                Лайк (<?php
                                                    $likes = $dbase -> read('likes', where: [
                                                        ['likeable_type', '=', 'comments'],
                        ...
                        foreach ($inner_comments as $inner_comment) {
                            ...
                            <article class="p-6 mb-3 lg:ml-12 text-base">
                                ...
                                <div class="peer flex justify-end mt-4 space-x-4">
                                    <a href="/post.php?id=<?php print($post -> getAttribute('id')) ?>&where=like_comment&comment_id=<?php print($inner_comment['id']); ?>" class="cursor-pointer flex items-center text-sm text-gray-500 hover:underline font-medium">
                                        <!-- Лайк -->
                                        Лайк (<?php
                                            $likes = $dbase -> read('likes', where: [
                                                ['likeable_type', '=', 'comments'],
